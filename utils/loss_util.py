...
class MultiLossFunction(nn.Module):
    def __init__(self, config, weights=None):
        ...
        # If weights are not provided, use equal weights for all losses
        if weights is None:
    ...
    def forward(self, prediction, target):
        ...
        total_loss = (self.weights[0]*inte_loss + self.weights[1]*grad_loss + 
                      self.weights[2]*msssim + self.weights[3]*l2_loss)

        #total_loss = (self.weights[0]*inte_loss + self.weights[3]*l2_loss)
