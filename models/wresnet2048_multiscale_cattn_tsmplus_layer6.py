...
class SReLU(nn.Module):
    def __init__(self, alpha=0.1, beta=0.8):
        ...
        self.alpha = nn.Parameter(torch.tensor(alpha)).cuda()  # controls the negative slope, as in Leaky ReLU
        self.beta = nn.Parameter(torch.tensor(beta)).cuda()    # controls smoothing
    
    def forward(self, x):
        # Leaky ReLU part for negative values
        negative_part = self.alpha * (x - F.relu(-x))
        
        # Regular ReLU for positive values
        positive_part = F.relu(x)
        
        # Smooth transition around zero using sigmoid function
        transition = self.beta * (x - F.relu(-self.beta * x)) * torch.sigmoid(x)
        
        return negative_part + positive_part + transition
